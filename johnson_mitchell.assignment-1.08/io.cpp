...
void io_display(dungeon_t *d)
{
  ...
  for (y = 0; y < 21; y++) {
    for (x = 0; x < 80; x++) {
      ...
      if(d->object_map[d->io_offset[dim_y] + y][d->io_offset[dim_x] + x]->being_used){
          pair_t obj_pos;
          obj_pos[dim_y] = d->io_offset[dim_y] + y;
          obj_pos[dim_x] = d->io_offset[dim_x] + x;
          if(can_see(d,character_get_pos(d->PC),obj_pos,1)){
              temp = false;
              attron(COLOR_PAIR(d->object_map[d->io_offset[dim_y] + y][d->io_offset[dim_x] + x]->color));

              mvaddch(y + 1,x,typeToSymbol(d->object_map[d->io_offset[dim_y] + y][d->io_offset[dim_x] + x]->type));
              attroff(COLOR_PAIR(d->object_map[d->io_offset[dim_y] + y][d->io_offset[dim_x] + x]->color));
