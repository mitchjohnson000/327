...
//  }
  ...
  if (atk == d->PC) {
    d->PC->base_damage = d->PC->base_damage + d->PC->damage->roll();
    int totalatk = 0;
    for(int i=0;i<d->PC->equipment.size();i++){
       totalatk += d->PC->equipment.at(i)->roll_dice();
    }
    d->PC->total_damage = d->PC->base_damage + totalatk;
    def->hp = def->hp - d->PC->total_damage;
    if(def->hp < 0){
        character_die(def);
        io_queue_message("You smite the %s", character_get_name(def));
    }
    d->PC->hp = d->PC->hp - def->damage->roll();
    if(d->PC->hp < 0){
        character_die(atk);
    }
    d->PC->base_damage = 200;
    d->PC->total_damage = 0;
  }else{

    d->PC->hp = d->PC->hp - atk->damage->roll();
    if(d->PC->hp < 0){
        character_die(atk);
    }
    d->PC->base_damage = 100;
    d->PC->total_damage = 0;   
    ...
    d->PC->total_damage = d->PC->base_damage + totalatk;
    atk->hp = atk->hp - d->PC->total_damage;
    if(atk->hp < 0){
        character_die(atk);
        io_queue_message("You smite the %s", character_get_name(atk));
    }
  }
...
void move_character(dungeon_t *d, character *c, pair_t next)
{
  if (charpair(next) &&
      ((next[dim_y] != character_get_y(c)) ||
       (next[dim_x] != character_get_x(c)))&& (c == d->PC ||charpair(next) == d->PC)) {
    do_combat(d, c, charpair(next));
  } else if(objpair(next) && c == d->PC){
      ...
      if(d->PC->inventory.size() < 10){
          d->PC->inventory.push_back(objpair(next));
