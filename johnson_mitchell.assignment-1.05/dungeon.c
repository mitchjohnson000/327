  ...
  while ((p = heap_remove_min(&h))) {
    ...
    if ((p->pos[dim_y] == to[dim_y]) && p->pos[dim_x] == to[dim_x]) {
      for (x = to[dim_x], y = to[dim_y];
        ...
        if (mapxy(x, y) != ter_floor_room && mapxy(x,y) != ter_floor_stair_up && mapxy(x,y)!= ter_floor_stair_down) {
...
//add stairs
  int x1 = d->rooms[0].position[dim_x];
  int y1 = d->rooms[0].position[dim_y];

  int i2,j2;

  for(i2 = 2;i2 < 5;i2++){
    for(j2 = 2;j2 < 4;j2++){
       d->map[y1 + j2][x1 + i2] = ter_floor_stair_up;
    }
  }

  x1 = d->rooms[d->num_rooms-1].position[dim_x];
  y1 = d->rooms[d->num_rooms-1].position[dim_y];
  for(i2 = 2;i2 < 5;i2++){
    for(j2 = 2;j2 < 4;j2++){
      d->map[y1 + j2][x1 + i2] = ter_floor_stair_down;
    }
  }
...
void render_dungeon(dungeon_t *d,int x,int y,WINDOW *local_win,int look_mode, pair_t * previous)
{
  pair_t p;

  if(look_mode){
    int x1 = *previous[dim_x] + x;
    int y1 = *previous[dim_y] + y;
    *previous[dim_x] = x1;
    *previous[dim_y] = y1;
  ...
  if(*previous[dim_x] + 81 > 164){
    *previous[dim_x] = 164 - 81;
  }
  if(*previous[dim_x] < 0){
    *previous[dim_x] = 0;
  }

  if(*previous[dim_y] + 21 > 104){
    *previous[dim_y] = 104 - 21;
  }
  ...
  for (p[dim_y] = *previous[dim_y]; p[dim_y] < *previous[dim_y] + 21; p[dim_y]++) {
    for (p[dim_x] = *previous[dim_x]; p[dim_x] < *previous[dim_x] + 81; p[dim_x]++) {
      if (charpair(p)) {
        waddch(local_win,charpair(p)->symbol);
      } else {
        switch (mappair(p)) {
        case ter_wall:
