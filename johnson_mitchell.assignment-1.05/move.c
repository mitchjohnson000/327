...
void do_moves(dungeon_t *d, int x, int y)
{
  pair_t next;
  character_t *c;
  event_t *e;
  ...
  if (pc_is_alive(d) && e->c == &d->pc) {
    ...
    next[dim_y] = y;
    next[dim_x] = x;
    // if(next[dim_x] + c->position[dim_x] > 164 || next[dim_x] + c->position[dim_x] < 0){
    //   next[dim_x] = 0;
    // }
    // if(next[dim_y] + c->position[dim_y] > 104 || next[dim_y] + c->position[dim_x] < 0){
    //   next[dim_y] = 0;
    // }
