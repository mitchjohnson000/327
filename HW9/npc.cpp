...
int rollDice2(dice d){
    int total = 0;
    int range = d.get_sides() - 1 + 1;
    int temp = rand() % range + 1;
    total = (d.get_number() * temp) + d.get_base();
    return total;
}
...
void gen_monsters(dungeon_t *d)
{
 ...
 // const static char symbol[] = "0123456789abcdef";
  ...
  for (i = 0; i < d->num_monsters; i++) {
    ...
    //memset(m, 0, sizeof (*m));
    int index = rand_range(0,d->monster_descriptions.size() -1);
    monster_description desc = d->monster_descriptions.at(index);
    ...
    m->color = desc.color.front();
    m->hitpoints = rollDice2(desc.hitpoints);
