...
int main(int argc, char *argv[])
{
 ...
 if (argc > 1) {
  ...
  int i1,j;
  for(i1 = 0; i1 < DUNGEON_Y;i1++){
      for(j = 0;j < DUNGEON_X;j++){
          object *obj = new object();
          obj->being_used = false;
          d.object_map[i1][j] = obj;
      }
  }
  
  for( i = 0; i < 100; i++){
      int index = (rand() % (d.object_descriptions.size() -1));
      object *obj = d.object_descriptions.at(index).genObject();
      int room = rand_range(1, d.num_rooms - 1);
      int y = rand_range(d.rooms[room].position[dim_y],
                            (d.rooms[room].position[dim_y] +
                             d.rooms[room].size[dim_y] - 1));
      int x = rand_range(d.rooms[room].position[dim_x],
